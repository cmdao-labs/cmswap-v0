...
import HeroSwapCard from "@/components/cmswap/HeroSwapCard"
import { redirect } from "next/navigation"

export default function Page() {
    redirect('/announcement')

    const canvasRef=React.useRef<HTMLCanvasElement>(null);const visualizationCanvasRef=React.useRef<HTMLCanvasElement>(null);React.useEffect(()=>{const canvas=canvasRef.current;if(!canvas)return;const ctx=canvas.getContext("2d");if(!ctx)return;const updateSize=()=>{canvas.width=window.innerWidth;canvas.height=400;};updateSize();window.addEventListener("resize",updateSize);const initVisualizationCanvas=()=>{const canvas=visualizationCanvasRef.current;if(!canvas)return;const ctx=canvas.getContext("2d");if(!ctx)return;canvas.width=500;canvas.height=300;let time=0;const speed=0.001;const drawVisualization=()=>{time+=speed;ctx.fillStyle="rgba(0, 0, 0, 0)";ctx.clearRect(0,0,canvas.width,canvas.height);const centerX=canvas.width/2;const centerY=canvas.height/2;const ringCount=15;const maxRadius=120;for(let i=0;i<ringCount;i++){const ringRadius=maxRadius*(0.3+(i/ringCount)*0.7);const ringTilt=Math.PI/4;const ringRotation=time*(0.2+i*0.05);const ringOpacity=0.7-(i/ringCount)*0.5;ctx.beginPath();const gradient=ctx.createLinearGradient(centerX-ringRadius,centerY,centerX+ringRadius,centerY);if(i%2===0){gradient.addColorStop(0,`rgba(0, 255, 150, ${ringOpacity})`);gradient.addColorStop(1,`rgba(0, 150, 255, ${ringOpacity})`);}else{gradient.addColorStop(0,`rgba(0, 200, 255, ${ringOpacity})`);gradient.addColorStop(1,`rgba(0, 255, 150, ${ringOpacity})`);}ctx.strokeStyle=gradient;ctx.lineWidth=1;for(let angle=0;angle<Math.PI*2;angle+=0.05){const rotatedAngle=angle+ringRotation;const x=centerX+Math.cos(rotatedAngle)*ringRadius;const y=centerY+Math.sin(rotatedAngle)*ringRadius*Math.cos(ringTilt);if(angle===0){ctx.moveTo(x,y);}else{ctx.lineTo(x,y);}}ctx.closePath();ctx.stroke();if(i%3===0){const particleCount=3;for(let j=0;j<particleCount;j++){const particleAngle=((Math.PI*2)/particleCount)*j+time*(1+i*0.2);const rotatedParticleAngle=particleAngle+ringRotation;const particleX=centerX+Math.cos(rotatedParticleAngle)*ringRadius;const particleY=centerY+Math.sin(rotatedParticleAngle)*ringRadius*Math.cos(ringTilt);ctx.beginPath();ctx.arc(particleX,particleY,2,0,Math.PI*2);ctx.fillStyle=i%2===0?"rgba(0, 255, 150, 0.9)":"rgba(0, 150, 255, 0.9)";ctx.fill();ctx.beginPath();const trailLength=Math.PI/8;for(let t=0;t<trailLength;t+=0.01){const trailAngle=rotatedParticleAngle-t;const trailX=centerX+Math.cos(trailAngle)*ringRadius;const trailY=centerY+Math.sin(trailAngle)*ringRadius*Math.cos(ringTilt);if(t===0){ctx.moveTo(trailX,trailY);}else{ctx.lineTo(trailX,trailY);}}const trailGradient=ctx.createLinearGradient(particleX,particleY,centerX+Math.cos(rotatedParticleAngle-trailLength)*ringRadius,centerY+Math.sin(rotatedParticleAngle-trailLength)*ringRadius*Math.cos(ringTilt));trailGradient.addColorStop(0,i%2===0?"rgba(0, 255, 150, 0.7)":"rgba(0, 150, 255, 0.7)");trailGradient.addColorStop(1,"rgba(0, 0, 0, 0)");ctx.strokeStyle=trailGradient;ctx.lineWidth=1.5;ctx.stroke();}}}ctx.beginPath();ctx.arc(centerX,centerY,6,0,Math.PI*2);const coreGradient=ctx.createRadialGradient(centerX,centerY,0,centerX,centerY,6);coreGradient.addColorStop(0,"rgba(255, 255, 255, 0.9)");coreGradient.addColorStop(1,"rgba(0, 200, 255, 0.7)");ctx.fillStyle=coreGradient;ctx.fill();ctx.beginPath();ctx.arc(centerX,centerY,15,0,Math.PI*2);const glowGradient=ctx.createRadialGradient(centerX,centerY,6,centerX,centerY,15);glowGradient.addColorStop(0,"rgba(0, 200, 255, 0.5)");glowGradient.addColorStop(1,"rgba(0, 200, 255, 0)");ctx.fillStyle=glowGradient;ctx.fill();ctx.beginPath();ctx.moveTo(centerX-20,centerY);ctx.lineTo(centerX+20,centerY);ctx.strokeStyle="rgba(255, 255, 255, 0.3)";ctx.lineWidth=0.5;ctx.stroke();ctx.beginPath();ctx.moveTo(centerX,centerY-20);ctx.lineTo(centerX,centerY+20);ctx.stroke();requestAnimationFrame(drawVisualization);};drawVisualization();};initVisualizationCanvas();return()=>{window.removeEventListener("resize",updateSize);};},[]);
    return (
            ...
            <main className="relative z-10 mt-[100px]">
                <div className="container mx-auto px-4 py-10">
                    <div className="max-w-4xl mx-auto text-center space-y-6">
                        <h1 className="mb-8 mx-auto text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight leading-tight font-mono relative flex flex-col items-start sm:items-center text-left sm:text-center">
